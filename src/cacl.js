const cacl = {
  ...
  context: class {
    ...
    circle(_x, _y, _r) {
      const [tX, tY, tX2, tY2, X, Y, X2, Y2] = this.boundingBox(_x - _r - 1, _y - _r - 1, _r * 2 + 2, _r * 2 + 2);
      const w = tX2 - tX;
      const h = tY2 - tY;

      const rp = (_r + 0.5) * (_r + 0.5);
      const rm = (_r - 0.5) * (_r - 0.5);

      for (let y = Y; y < Y2; y++) {
        let pos = (X + y * this.width) << 2;
        const y2 = (y - _y + 0.5) ** 2;
        for (let x = X; x < X2; x++, pos += 4) {
          const dist2 = (x - _x + 0.5) ** 2 + y2;
          if (dist2 < rm) {
            this.imageData[pos + 0] = this.fillStyle.a * this.fillStyle.r * 65535 + (1 - this.fillStyle.a) * this.imageData[pos + 0];
            this.imageData[pos + 1] = this.fillStyle.a * this.fillStyle.g * 65535 + (1 - this.fillStyle.a) * this.imageData[pos + 1];
            this.imageData[pos + 2] = this.fillStyle.a * this.fillStyle.b * 65535 + (1 - this.fillStyle.a) * this.imageData[pos + 2];
          ...
          if (dist2 >= rp) continue;
          const A = _r + 0.5 - Math.sqrt(dist2);
          this.imageData[pos + 0] = this.fillStyle.a * A * this.fillStyle.r * 65535 + (1 - this.fillStyle.a * A) * this.imageData[pos + 0];
          this.imageData[pos + 1] = this.fillStyle.a * A * this.fillStyle.g * 65535 + (1 - this.fillStyle.a * A) * this.imageData[pos + 1];
          this.imageData[pos + 2] = this.fillStyle.a * A * this.fillStyle.b * 65535 + (1 - this.fillStyle.a * A) * this.imageData[pos + 2];
          this.imageData[pos + 3] = this.fillStyle.a * A * 65535 + (1 - this.fillStyle.a * A) * this.imageData[pos + 3];
        }
      }
    }
    ...
    fillPath() {
      for (let i = 0; i < this.pathLines.length; i += 4) {
        this.line(...this.pathLines.slice(i, i + 4));
      }
      this.fillPathAliased();
    }

    line(x1, y1, x2, y2) {
      const tempA = this.fillStyle.a;
      ...
      const lineVal = 2 * ((Math.SQRT2 - 1) / 2) ** 2;

      if (a1 === 0) {
        this.fillRect(Math.min(x1, x2), y1, Math.abs(x1 - x2), 1);
        return;
      }
      if (b1 === 0) {
        this.fillRect(x1, Math.min(y1, y2), 1, Math.abs(y1 - y2));
        return;
      }
      ...
      if (maxX - minX < maxY - minY) {
        ...
        if (y1 < y2) {
          let y = y1;
          let x = (c1 - b1 * y) / a1;
          const Y2 = (Math.floor(y) - y1) ** 2;

          this.fillStyle.a = 1 - Math.sqrt(Y2 + (Math.floor(x) - x1) ** 2);
          if (this.fillStyle.a < 0) this.fillStyle.a = 0;
          this.fillRect(Math.floor(x), Math.floor(y));
          this.fillStyle.a = 1 - Math.sqrt(Y2 + (Math.ceil(x) - x1) ** 2);
          if (this.fillStyle.a < 0) this.fillStyle.a = 0;
          this.fillRect(Math.ceil(x), Math.floor(y));

          y = y2;
          x = (c1 - b1 * y) / a1;
          const Y2_ = (Math.ceil(y) - y1) ** 2;

          this.fillStyle.a = 1 - Math.sqrt(Y2_ + (Math.floor(x) - x2) ** 2);
          if (this.fillStyle.a < 0) this.fillStyle.a = 0;
          this.fillRect(Math.floor(x), Math.ceil(y));
          this.fillStyle.a = 1 - Math.sqrt(Y2_ + (Math.ceil(x) - x2) ** 2);
          if (this.fillStyle.a < 0) this.fillStyle.a = 0;
          this.fillRect(Math.ceil(x), Math.ceil(y));
        } else {
          let y = y1 + 1;
          let x = (c1 - b1 * y) / a1;
          const Y2 = (Math.floor(y) - y1) ** 2;
        ...
        // line
        for (let y = Math.floor(minY + 1); y < maxY; y++) {
          const x = (c1 - b1 * y) / a1;

          this.fillStyle.a = 1 - Math.abs(Math.floor(x) - x);
          this.fillRect(Math.floor(x), Math.round(y));
          this.fillStyle.a = 1 - Math.abs(Math.ceil(x) - x);
          this.fillRect(Math.ceil(x), Math.round(y));
        }
      } else {
        // sides of line
        if (x1 < x2) {
          let x = x1;
          let y = (c1 - a1 * x) / b1;
          const X2 = (Math.floor(x) - x1) ** 2;

          this.fillStyle.a = 1 - Math.sqrt((Math.floor(y) - y1) ** 2 + X2);
          if (this.fillStyle.a < 0) this.fillStyle.a = 0;
          this.fillRect(Math.floor(x), Math.floor(y));
          this.fillStyle.a = 1 - Math.sqrt((Math.ceil(y) - y1) ** 2 + X2);
          if (this.fillStyle.a < 0) this.fillStyle.a = 0;
          this.fillRect(Math.floor(x), Math.ceil(y));

          x = x2;
          y = (c1 - a1 * x) / b1;
          const X2_ = (Math.ceil(x) - x2) ** 2;

          this.fillStyle.a = 1 - Math.sqrt((Math.floor(y) - y2) ** 2 + X2_);
          if (this.fillStyle.a < 0) this.fillStyle.a = 0;
          this.fillRect(Math.ceil(x), Math.floor(y));
          this.fillStyle.a = 1 - Math.sqrt((Math.ceil(y) - y2) ** 2 + X2_);
          if (this.fillStyle.a < 0) this.fillStyle.a = 0;
          this.fillRect(Math.ceil(x), Math.ceil(y));
        } else {
          let x = x1 + 1;
        ...
        // line
        for (let x = Math.floor(minX + 1); x < maxX; x++) {
          const y = (c1 - a1 * x) / b1;

          this.fillStyle.a = 1 - Math.abs(Math.floor(y) - y);
          this.fillRect(Math.round(x), Math.floor(y));
          this.fillStyle.a = 1 - Math.abs(Math.ceil(y) - y);
          this.fillRect(Math.round(x), Math.ceil(y));
        }
      }

      this.fillStyle.a = tempA;
    }
    ...
    background(r = -1, g = -1, b = 0, a = 255) {
      if (typeof r === 'string' && cacl.CSSColors.hasOwnProperty(r)) {
        this.fillStyle = cacl.CSSColors[r];
      } else if (r >= 0) {
        if (g === -1) {
          this.fill(r, r, r);
        } else {
  ...
  weightedAverage: (values, weights) => {
    ...
    for(let i = 0; i < values.length; i++) {
      ans += values * weights[i];
      sum += weights[i];
    }
    if(sum === 0) return 0;
    return ans / sum;
  },
  weightedArrayAverage: (values, weights) => {
  ...
  weightedAverageColors: (colors, weights) => {
    let r = 0;
    let g = 0;
    let b = 0;
    let a = 0;
  ...
  pointOnBezier: (x1, y1, c1x, c1y, c2x, c2y, x2, y2, x, resolution = 20) => {
    ...
    for(let i = 0; i < resolution; i++) {
      b = this.cubicBezier(x1, c1x, c2x, x2, at);
      if(b > x) at -= inc;
      else at += inc;
      inc /= 2;
    }
    return this.cubicBezier(y1, c1y, c2y, y2, at);
  },
  easeValue: (atTime, fromTime, toTime, fromValue, toValue, easeA, easeB, beforeFromTime = false, beforeFromValue = false, afterToTime = false, afterToValue = false, resolution = 20) => {
